...
exports.hexToString = function(hex,appended0x){
	...
	try{
		...
		let buff = new Uint8Array((hex.length / 2) - (appended0x === false ? 0 : 1));
		let ii = 0;
		for (let i=(appended0x === false ? 0 : 2); i < hex.length; i += 2){
			buff[ii++] = hexToNum[hex.substr(i,2).toLowerCase()];
		}
		return String.fromCharCode(...buff); // decodeURIComponent throws on invalid UTF-8. Use Latin1 instead.
	}
}
